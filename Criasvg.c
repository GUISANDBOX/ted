...
void criatexto(int i, double x, double y, char corb[], char corp[], char a, char txto[], FILE *f){
    if (a=='i') {
        fprintf(f,"<text x=\"%lf\" y=\"%lf\" stroke=\"%s\" fill=\"%s\" fill-opacity=\"0.5\" text-anchor=\"start\" >%s</text>\n",x,y,corb,corp,txto);
    }
    else if (a=='f') {
        fprintf(f,"<text x=\"%lf\" y=\"%lf\" stroke=\"%s\" fill=\"%s\" fill-opacity=\"0.5\" text-anchor=\"end\" >%s</text>\n",x,y,corb,corp,txto);
    }
    else {
        fprintf(f,"<text x=\"%lf\" y=\"%lf\" stroke=\"%s\" fill=\"%s\" fill-opacity=\"0.5\" text-anchor=\"middle\" >%s</text>\n",x,y,corb,corp,txto);
    }
}